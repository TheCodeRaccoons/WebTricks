...
describe("CMSFilter", () => {
  ...
  beforeEach(() => {
    ...
    ({
      CMSFilter,
      InitializeCMSFilter,
    } = require("../Dist/WebflowOnly/CMSFilter.js"));
  });
  ...
  function buildBasicDOM({
    ...
    const paginationMarkup = withPagination
      ...
      : "";
    const advancedAttr = advanced
      ? 'wt-cmsfilter-filtering="advanced" wt-cmsfilter-class="is-active"'
      : "";
    document.body.innerHTML = `
      ...
      <div wt-cmsfilter-element="empty" ${emptyMax !== undefined ? `wt-cmsfilter-empty-max="${emptyMax}"` : ""} style="display:none;">No results</div>
      ${paginationMarkup}
    `;
  }
  ...
  test("initializes and caches items, pushes instance", () => {
    ...
    const countText =
      instance.resultCount.textContent || String(instance.filteredItems.length);
  ...
  test("global search via * category filters list items", () => {
    ...
    instance.ApplyFilters();
    expect(instance.filteredItems.length).toBe(1);
    expect(instance.filteredItems[0].textContent.toLowerCase()).toContain(
      "gamma",
    );
  });
  ...
  test("range filtering narrows items between from/to values", () => {
    ...
    const priceFrom = document.querySelector(
      '[wt-cmsfilter-category="Price"][wt-cmsfilter-range="from"] input',
    );
    const priceTo = document.querySelector(
      '[wt-cmsfilter-category="Price"][wt-cmsfilter-range="to"] input',
    );
  ...
  test("clear all resets filters and shows all items again", () => {
    ...
    InitializeCMSFilter();
    const form = document.querySelector('[wt-cmsfilter-element="filter-form"]');
    const alpha = Array.from(form.querySelectorAll("label")).find((l) =>
  ...
  test("advanced filtering hides unavailable checkboxes then restores after clearing", () => {
    ...
    const labels = Array.from(
      form.querySelectorAll('label[wt-cmsfilter-category="Category"]'),
    );
  ...
  test("tag template displays active filters and can remove a tag", () => {
    ...
    InitializeCMSFilter();
    const form = document.querySelector('[wt-cmsfilter-element="filter-form"]');
    const beta = Array.from(form.querySelectorAll("label")).find((l) =>
      l.textContent.includes("Beta"),
    );
    ...
    const remove = tagsContainer.querySelector(
  ...
  test("empty element shows when filtered result count is less than or equal to wt-cmsfilter-empty-max", () => {
    ...
    expect(instance.filteredItems.length).toBe(1);
    expect(instance.emptyElement.style.display).toBe("block");
  });

  test('wt-cmsfilter-element="empty" works by itself', () => {
    buildBasicDOM();
    InitializeCMSFilter();
    const instance = window.webtricks[0].CMSFilter;

    expect(instance.emptyElement).toBeTruthy();
    ...
    expect(instance.filteredItems.length).toBe(0);
    expect(instance.emptyElement.style.display).toBe("block");
  });

  test("missing wt-cmsfilter-empty-max defaults to 0", () => {
    buildBasicDOM();
    InitializeCMSFilter();
    const instance = window.webtricks[0].CMSFilter;
