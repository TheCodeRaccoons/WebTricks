...
class CMSFilter {
  constructor() {
    ...
    this.filterElements = this.filterForm.querySelectorAll(
    ...
    this.debounceDelay = parseInt(
      this.filterForm.getAttribute("wt-cmsfilter-debounce") || "300",
    );

    //TAG elements
    this.tagTemplate = document.querySelector(
      '[wt-cmsfilter-element="tag-template"]',
    );
    this.tagTemplateContainer = this.tagTemplate
      ? this.tagTemplate.parentElement
      : null;
    ...
    this.paginationWrapper =
      document.querySelector('[wt-cmsfilter-element="pagination-wrapper"]') ||
      null;
    this.loadMode =
      this.listElement.getAttribute("wt-cmsfilter-loadmode") || "load-all";
    this.previousButton = document.querySelector(
      '[wt-cmsfilter-pagination="prev"]',
    );
    this.nextButton = document.querySelector(
      '[wt-cmsfilter-pagination="next"]',
    );
    this.customNextButton = document.querySelector(
      '[wt-cmsfilter-element="custom-next"]',
    );
    this.customPrevButton = document.querySelector(
      '[wt-cmsfilter-element="custom-prev"]',
    );

    this.paginationcounter = document.querySelector(
      '[wt-cmsfilter-element="page-count"]',
    );
    this.activeFilterClass = this.filterForm.getAttribute("wt-cmsfilter-class");
    this.clearAll = document.querySelector(
      '[wt-cmsfilter-element="clear-all"]',
    );
    this.sortOptions = document.querySelector(
      '[wt-cmsfilter-element="sort-options"]',
    );
    this.resultCount = document.querySelector(
      '[wt-cmsfilter-element="results-count"]',
    );
    this.emptyElement = document.querySelector(
      '[wt-cmsfilter-element="empty"]',
    );
    ...
    this.resetIx2 =
      this.listElement.getAttribute("wt-cmsfilter-resetix2") || false;
  ...
  async init() {
    ...
    if (this.paginationWrapper) {
      ...
      if (
        ...
        this.paginationcounter !=
          this.paginationWrapper.querySelector(".w-page-count")
      ) {
  ...
  configureRangeSliders() {
    ...
    rangeSliders.forEach((slider) => {
      ...
      if (!hasManualMin) {
        slider.setAttribute(
          "wt-rangeslider-min",
          this.dataRanges[datasetCategory].min.toString(),
        );
      }
      if (!hasManualMax) {
        slider.setAttribute(
          "wt-rangeslider-max",
          this.dataRanges[datasetCategory].max.toString(),
        );
      }
      ...
      if (!slider.hasAttribute("wt-rangeslider-steps")) {
        const range =
          this.dataRanges[datasetCategory].max -
          this.dataRanges[datasetCategory].min;
        const defaultSteps =
          range > 1000 ? 100 : range > 100 ? 10 : range > 10 ? 1 : 0.1;
  ...
  async LoadAllItems() {
    ...
    const links = this.generatePaginationLinksFromString(
      paginationPages.innerText,
      baseLink.href,
    );
      ...
      try {
        ...
        if (htmlDoc) {
          const cards = Array.from(
            htmlDoc.querySelector('[wt-cmsfilter-element="list"]')?.children ||
              [],
          );
  ...
  RenderItems() {
    ...
    if (this.paginationWrapper) {
      if (this.loadMode === "load-all") {
        ...
        const currentPage = this.filteredItems.slice(
          currentSlice,
          currentSlice + this.itemsPerPage,
        );
  ...
  SortItems() {
    ...
    this.filteredItems = this.filteredItems.filter(
      (item) => !item.hasAttribute("wt-renderstatic-element"),
    );
        ...
        if (input.type === "text") {
          ...
          if (rangeType === "from" || rangeType === "to") {
            ...
            if (Number.isFinite(value)) {
              const datasetCategory = this.GetDataSet(category);
              const ranges = this.dataRanges
                ? this.dataRanges[datasetCategory]
                : null;
              // Determine default for comparison without mutating attributes here
              let numericDefault = parseFloat(
                input.getAttribute("wt-cmsfilter-default"),
              );
  ...
  captureOriginalDisplayStyles() {
    this.filterElements.forEach((element) => {
      const istoggle = element.querySelector(
  ...
  ToggleEmptyState() {
    if (this.emptyElement) {
      if (this.filteredItems.length <= this.emptyMaxCount) {
    ...
    filterTags.forEach((tag) => {
      if (this.activeFilters[tag].length !== 0) {
        this.activeFilters[tag].forEach((filterValue) => {
          ...
          const tagText = newTag.querySelector(
            '[wt-cmsfilter-element="tag-text"]',
          );
          const showTagCategory =
            newTag.getAttribute("wt-cmsfilter-tag-category") || "true";
          const tagRemove = newTag.querySelector(
            '[wt-cmsfilter-element="tag-remove"]',
          );
  ...
  RemoveActiveTag(_tag, filterTag, value) {
    const categoryElements = this.filterForm.querySelectorAll(
      `[wt-cmsfilter-category="${filterTag}"]`,
    );
    const advancedFiltering = this.filterForm.getAttribute(
      "wt-cmsfilter-filtering",
    );
    categoryElements.forEach((categoryElement) => {
      const input =
        categoryElement.tagName === "INPUT"
          ? categoryElement
    ...
    this.activeFilters[filterTag] = this.activeFilters[filterTag].filter(
      (filter) => filter !== value,
    );
  ...
  GetResults() {
    if (this.activeFilters) {
      let currActive = Object.values(this.activeFilters).filter(
    ...
    if (this.allItems) {
      ...
      let elements = this.allItems.filter(
        (item) => !item.hasAttribute("wt-renderstatic-element"),
      );
